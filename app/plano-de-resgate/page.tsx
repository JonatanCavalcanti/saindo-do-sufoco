...
// Em produção estes dados vêm de `fixed_expenses` (is_essential=true),
// `cards`/`invoices` (rotativo) e `external_debts` — aqui, exemplo fixo.
const MOCK_INCOME = 6200;
const MOCK_ESSENTIALS: EssentialExpense[] = [
  { id: "1", name: "Aluguel", amount: 1800 },
  { id: "2", name: "Água, luz e internet", amount: 420 },
  { id: "3", name: "Alimentação", amount: 900 },
];
const MOCK_DEBTS: UnifiedDebt[] = [
  { id: "d1", source: "cartao", name: "Nubank (rotativo)", balance: 2180, monthlyRate: 0.145, minimumPayment: 327 },
  { id: "d2", source: "cartao", name: "Inter Black (rotativo)", balance: 4950, monthlyRate: 0.132, minimumPayment: 742 },
  { id: "d3", source: "emprestimo", name: "Empréstimo pessoal Caixa", balance: 6400, monthlyRate: 0.028, minimumPayment: 540 },
];
...
export default function RescuePlanPage() {
  ...
  const plan = useMemo(
    () =>
      buildRescuePlan({
        income: MOCK_INCOME,
        essentialExpenses: MOCK_ESSENTIALS,
        debts: MOCK_DEBTS,
        method,
      }),
    [method]
      ...
      <div className="rounded-card bg-white border border-moss-200 p-5 mb-4">
        ...
        <div className="w-full h-2 rounded-full bg-base-200 overflow-hidden flex">
          <div
            ...
            style={{ width: `${Math.min((plan.essentialReserve / plan.income) * 100, 100)}%` }}
          />
